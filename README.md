# Домашка №4 (заявочка на оценку 5)


**На "3".**  
Скачала файлы с урока, запустила в докере, продемонстрировала выполнение
запросов на insert, find, update, delete через терминал.

**На "4".**  
Взяла за основу схему хранения данных из предыдущей домашки (продажа машин). Заполнила
базу данных по схеме и продемонстрировала запросы. Выполнила
запросы find, update, delete с различными опциями и пояснила, что за
опции были использованы.

**На "5".**  
То же, что и в 4.
Добавила запросы с агрегацией из нескольких коллекций. Разобралась с
оператором $explain, продемонстрировала его работу на примере
запроса и прокомментировала.
